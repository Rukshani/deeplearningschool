...
![loss2d.gif]({{site.baseurl}}/media/loss2d.gif)![loss3d.gif]({{site.baseurl}}/media/loss3d.gif)

{:.image-caption}
Top: 1-variable gradient descent. Bottom: multivariable gradient descent, with
...
is the learning rate, and it affects how quickly $m$ changes. Set $\alpha$ (learning rate) too small and gradient descent will take a long time to reach the minimum. However, set $\alpha$ too large and gradient descent might even diverge.
...
Left: Stochastic Gradient Descent Algorithm. Right: SGD in practice. Note how SGD is more “jerky” and random, but still converges to around the same place 
